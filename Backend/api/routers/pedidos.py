...
from services import pedidos_services
...
def crear_pedido(payload: Dict[str, Any]):
    try:
        pedido = pedidos_services.crear_pedido(payload)
...
def listar_pedidos(cliente_id: Optional[int] = Query(None)):
    if cliente_id:
        pedidos = pedidos_services.listar_pedidos_por_cliente(cliente_id)
    else:
...
def obtener_pedido(pedido_id: int):
    try:
        pedido = pedidos_services.obtener_pedido_por_id(pedido_id)
        return pedido.to_dict()
...
def actualizar_pedido(pedido_id: int, payload: Dict[str, Any]):
    try:
        pedido = pedidos_services.actualizar_pedido(pedido_id, payload)
        return pedido.to_dict()
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    except LookupError as e:
...
def eliminar_pedido(pedido_id: int):
    try:
        pedidos_services.eliminar_pedido(pedido_id)
        return None
    except LookupError as e:
        raise HTTPException(status_code=404, detail=str(e))
    except Exception as e:
