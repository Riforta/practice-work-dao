...
def crear_inscripcion(payload: Dict[str, Any]):
    try:
        insc = inscripciones_service.crear_inscripcion(payload)
        return insc.to_dict()
...
def listar_inscripciones():
    insc = inscripciones_service.listar_inscripciones()
...
def obtener_inscripcion(inscripcion_id: int):
    try:
        insc = inscripciones_service.obtener_inscripcion_por_id(inscripcion_id)
        return insc.to_dict()
...
def actualizar_inscripcion(inscripcion_id: int, payload: Dict[str, Any]):
    try:
        insc = inscripciones_service.actualizar_inscripcion(inscripcion_id, payload)
        return insc.to_dict()
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    except LookupError as e:
...
def eliminar_inscripcion(inscripcion_id: int):
    try:
        inscripciones_service.eliminar_inscripcion(inscripcion_id)
        return None
    except LookupError as e:
        raise HTTPException(status_code=404, detail=str(e))
    except Exception as e:
