...
from services import equipo_miembros_services
...
def agregar_miembro(payload: Dict[str, Any]):
    try:
        equipo_miembros_services.agregar_miembro(payload)
        return {"status": "ok"}
...
def listar_miembros(equipo_id: Optional[int] = Query(None)):
    if not equipo_id:
        return []
    miembros = equipo_miembros_services.listar_miembros_por_equipo(equipo_id)
...
def eliminar_miembro(equipo_id: int, cliente_id: int):
    try:
        equipo_miembros_services.eliminar_miembro(equipo_id, cliente_id)
        return None
