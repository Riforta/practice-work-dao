...
export default function RegistroCanchaPadel() {
  const [action, setAction] = useState("R");
  const [errorMessage, setErrorMessage] = useState("");
  
  const { register, handleSubmit, formState: { errors }, reset } = useForm<FormData>({
    defaultValues: { tipo_deporte: "Padel", activa: true }
  });
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      const coincidencias = await service.getCanchaPadelByName(data.nombre);
      ...
      await service.creatCanchaPadel(data);
