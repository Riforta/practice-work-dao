...
export default function ModificarCanchaBasquet() {
  ...
  useEffect(() => {
    const fetchCancha = async () => {
      ...
      setError('');
      try {
        const data = await service.getByIdBasquet(Number(id));
        setValue('nombre', data.nombre);
        setValue('descripcion', data.descripcion || '');
        setValue('activa', data.activa);
        setValue('precio_hora', data.precio_hora);
        setValue('tipo_deporte', 'basquet');
      } catch (err: any) {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      if (exists) {
        setError('Ya existe otra cancha de básquet con ese nombre.');
        setLoading(false);
        return;
      }
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        <header className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <h1 className="text-3xl font-bold">Modificar Cancha de Básquet</h1>
          </div>
          <button
