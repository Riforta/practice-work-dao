...
export default function ConsultarCanchaBasquet() {
  ...
  const fetchCanchas = async (searchTerm = '') => {
    ...
    try {
      let data;
      if (searchTerm) {
        data = await service.getCanchaBasquetByName(searchTerm);
      } else {
        data = await service.getAllCanchasBasquet();
      }
      setRows(data);
    } catch (err) {
      console.error('Error cargando canchas:', err);
      setError('No se pudieron cargar las canchas de básquet.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id: number) => {
    ...
    setDeleteId(id);
    try {
      await service.deleteCanchaBasquet(id);
    ...
    <div className="min-h-screen bg-slate-950 text-white px-4 py-10">
      <div className="max-w-7xl mx-auto space-y-6">
        <header className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <p className="text-sm uppercase tracking-widest text-emerald-200">Canchas de Básquet</p>
            <h1 className="text-3xl font-bold">Gestión de Canchas de Básquet</h1>
          ...
          <div className="flex gap-3">
            ...
            <button
              onClick={() => navigate('/canchas/basquet/RegistrarCancha')}
        ...
        <section className="rounded-2xl border border-white/10 bg-white/5 p-4">
          ...
          {loading ? (
            ...
            <p className="text-center text-emerald-100 py-8">
              {filter ? 'No se encontraron canchas con ese criterio.' : 'No hay canchas de básquet registradas.'}
            </p>
          ) : (
